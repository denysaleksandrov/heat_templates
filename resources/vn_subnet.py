...
class HeatVnSubnet(contrail.ContrailResource):
    PROPERTIES = (
        ...
        ADDR_FROM_START, ENABLE_DHCP, DNS_NAMESERVERS, ALLOCATION_POOLS,
        HOST_ROUTES,
    ) = (
        'name', 'network', 'ip_prefix', 'default_gateway', 'ipam',
        'addr_from_start', 'enable_dhcp', 'dns_nameservers', 'allocation_pools',
    ...
    properties_schema = {
        ...
        ADDR_FROM_START: properties.Schema(
            properties.Schema.STRING,
            _('Strat allocating from the beginning of a range.'),
            default=True,
            constraints=[
    ...
    attributes_schema = {
        DNSES: attributes.Schema(
            _("DNS servers confgiured")
        ),
    ...
    def _resolve_attribute(self, name):
        try:
            netid = self.properties[self.NETWORK]
            net = self.vnc_lib().virtual_network_read(id=netid)
        except Exception as e:
            self._ignore_not_found(e)
            return ''
        ipams = [ subnet['attr'].get_ipam_subnets() for subnet in net.get_network_ipam_refs() ]
        if name == self.DNSES:
            dnses = [ ','.join(ipam.get_dns_nameservers()) for subnet in ipams for ipam in subnet ]
            return ','.join(dnses)
    ...
    def handle_create(self):
        ...
        if self.properties[self.ENABLE_DHCP] == "True":
            ...
            enable_dhcp = False
        if self.properties[self.ADDR_FROM_START] == "True":
        ...
        if self.properties[self.DNS_NAMESERVERS]:
            dhcp_option = [{'dhcp_option_value': i, 'dhcp_option_value_bytes': None,
                        'dhcp_option_name': 6} for i in self.properties[self.DNS_NAMESERVERS]]
            option_list = vnc_api.DhcpOptionsListType(dhcp_option=dhcp_option)
            subnet.set_dhcp_option_list(option_list)           
        if self.properties[self.HOST_ROUTES]:
            pass
            # if you want to create host routes, do it here. 
            # approach is more or less the same as with dns_nameservers
